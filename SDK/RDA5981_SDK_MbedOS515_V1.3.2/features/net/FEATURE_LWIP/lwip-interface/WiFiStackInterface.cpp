...
int WiFiStackInterface::connect(const char *ssid, const char *pass, const char *bssid, nsapi_security_t security, uint8_t channel)
{
    ...
    if (bss_list == NULL) {
        WIFISTACK_PRINT("malloc buf fail\r\n");
        goto exit;
    }
    rda5981_get_scan_result(bss_list, scan_res);
    for (i=0; i<scan_res; ++i) {
        WIFISTACK_PRINT("ssid:%s:%s, BSSID:%02x:%02x:%02x : %02x:%02x:%02x\r\n", bss_list[i].SSID, SSID, \
            bss_list[i].BSSID[0], bss_list[i].BSSID[1], bss_list[i].BSSID[2], bssid[0], bssid[1], bssid[2]);
        if (strcmp(bss_list[i].SSID, SSID) == 0 && ((bssid == NULL) || mbed_lwip_same_mac(bssid, (const char*)bss_list[i].BSSID))){
            memcpy(&_bss, &bss_list[i], sizeof(rda5981_scan_result));
            find = 1;
            break;
        }
     }
...
int WiFiStackInterface::reconnect()
{
    ...
    for (i=0; i<scan_res; ++i) {
        WIFISTACK_PRINT("ssid:%s:%s, BSSID:%02x:%02x:%02x : %02x:%02x:%02x\r\n", bss_list[i].SSID, SSID, \
            bss_list[i].BSSID[0], bss_list[i].BSSID[1], bss_list[i].BSSID[2], BSSID[0], BSSID[1], BSSID[2]);
        if ((strcmp(bss_list[i].SSID, SSID) == 0)  && (mbed_lwip_zero_mac(BSSID) || mbed_lwip_same_mac(BSSID, (const char*)bss_list[i].BSSID))){
            memcpy(&_bss, &bss_list[i], sizeof(rda5981_scan_result));
            find = 1;
            break;
        }
...
int WiFiStackInterface::start_ap(const char *ssid, const char *pass, int channel)
{
    if(_ip_address_ap[0] == 0){
        ...
        return mbed_lwip_startap(ssid, pass, mbed_ip_addr, mbed_msk_addr, mbed_gw_addr, mbed_dhcp_start, mbed_dhcp_end, channel);
    }else{
        ...
        return mbed_lwip_startap(ssid, pass, (const char *)&ip, (const char *)&mask, (const char *)&gw, (const char *)&dhcps, (const char *)&dhcpe, channel);
    }
}
