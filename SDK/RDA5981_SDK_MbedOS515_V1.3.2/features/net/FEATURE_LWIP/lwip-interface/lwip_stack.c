        ...
        bool dhcp, const char *ip, const char *netmask, const char *gw)
{
  ...
  lwip_netif.ip6_autoconfig_enabled = 1;
    ...
    ret = rda_msg_put(wland_msgQ, WLAND_CONNECT, osWaitForever);
    if (ret)
        return NSAPI_ERROR_AUTH_FAILURE;

    if (!netif_is_link_up(&lwip_netif)) {
        ret = sys_arch_sem_wait(&lwip_netif_linked, 5000);
...
#endif
#endif
    rda_msg_put(wland_msgQ, WLAND_AUTO_RATE, osWaitForever);
/*
                        ...
                        const char *dhcp_start, const char *dhcp_end, int channel)
{
    ...
    if (!mbed_lwip_get_mac_address(1)) {
        ...
        //netif_set_link_callback(&lwip_netif_ap, mbed_lwip_netif_link_irq);
    ...
    if(channel>0 && channel<14)
        gchannel_ap = channel;
    rda_msg_put(wland_msgQ, WLAND_STARTAP, osWaitForever);
...
nsapi_error_t mbed_lwip_stopap(u8_t state)
{
    ...
    if(state == 0)
        rda_msg_put(wland_msgQ, WLAND_STOPAP, osWaitForever);
    dhcps_deinit();
...
nsapi_error_t mbed_lwip_bringdown(void)
{
    ...
    rda_msg_put(wland_msgQ, WLAND_DISCONNECT, osWaitForever);
#if LWIP_IPV6
    mbed_lwip_clear_ipv6_addresses(&lwip_netif);
#endif
